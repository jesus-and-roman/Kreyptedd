...
Deno.serve(async (req) => {
  ...
  const incoming = (incomingRaw ?? []).map(r => ({ id: r.id, from_user_id: r.from_user, from_username: r.app_users?.username }));
  ...
  const { data: blockedRaw } = await supabase
    .from("blocked_users")
    .select("blocked_id, app_users!blocked_users_blocked_id_fkey(username)")
    .eq("user_id", userId);

  const blocked = (blockedRaw ?? []).map(b => ({ id: b.blocked_id, username: b.app_users?.username }));
